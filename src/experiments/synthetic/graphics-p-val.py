...
# Cargar cada CSV en un DataFrame y almacenarlos en una lista
dfs = [pd.read_csv(f) for f in files]


pattern = r'^(HD_|SHD_|F1_)'
...
N = len(order)
x_values = [0.1 / (2**i) for i in range(N)][::-1]  # biggest df gets smallest exponent
...
for metric in metrics:
    ...
    plt.gca().set_xticklabels([f"{0.003125:.6f}", f"{0.00625:.5f}", f"{0.0125:.4f}", f"{0.025:.3f}", f"{0.05:.2f}", f"{0.1:.1f}" ])
