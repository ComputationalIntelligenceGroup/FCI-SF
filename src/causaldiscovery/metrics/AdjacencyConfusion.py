...
class AdjacencyConfusion:
    ...
    def get_adj_precision(self):
        return self.__adjTp / (self.__adjTp + self.__adjFp) if self.__adjTp + self.__adjFp != 0 else np.nan

    def get_adj_recall(self):
        return self.__adjTp / (self.__adjTp + self.__adjFn) if self.__adjTp + self.__adjFn != 0 else np.nan
